...
function fromWireBuffers(buffers) {
  ...
  return buffers.map((buffer) => {
    if (ArrayBuffer.isView(buffer)) {
      // Node only pools small allocations, and a view that already spans its
      // whole ArrayBuffer is therefore not a window onto the pool — there is
      // nothing of anyone else's to expose, and nothing to copy. That case is
      // exactly the large payload where the copy would cost: an image frame, a
      // canvas update, a big binary trait.
