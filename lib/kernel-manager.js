...
const openExternal = (url) => {
  void lumine.shell.openExternal(url).catch((error) => {
    lumine.notifications.addWarning("Unable to open the Jupyter documentation.", {
...
class KernelManager {
  ...
  getKernelStartDirectory(fileOwner) {
    const currentPath = getEditorDirectory(fileOwner);
    let projectPath;
    ...
    return projectPath != null ? projectPath : currentPath;
  }
  ...
  startKernel(kernelSpec, grammar, editor, filePath, onStarted, bindingOptions = {}) {
    const displayName = kernelSpec.display_name;
    const startingKernelKey = bindingOptions.startKey || displayName;
    // if kernel startup already in progress don't start additional kernel
    if (store.startingKernels.has(startingKernelKey)) {
      return;
    }
    store.startKernel(startingKernelKey);
    log("KernelManager: startKernel:", displayName);
    const kernelStartDir = this.getKernelStartDirectory(bindingOptions.bindingOwner || editor);
    const options = {
      cwd: kernelStartDir,
      startingKernelKey,
    ...
    const transport = new ZMQKernel(kernelSpec, grammar, options, () => {
      ...
      if (!bindingOptions.deferRegistration) {
  ...
  async getKernelSpecForGrammar(grammar, editor) {
    ...
    return new Promise((resolve) => {
      ...
      const picker = this.kernelPicker;
      const settle = (kernelSpec) => {
        if (picker.onConfirmed === confirm) picker.onConfirmed = null;
        if (picker.onCancelled === cancel) picker.onCancelled = null;
        resolve(kernelSpec);
      };
      const confirm = (kernelSpec) => settle(kernelSpec);
      const cancel = () => settle(null);
      picker.onConfirmed = confirm;
      picker.onCancelled = cancel;

      picker.toggle({ editor });
    });
  }
          ...
          buttons: [
            {
              ...
              onDidClick: () =>
                openExternal("https://nteract.gitbooks.io/jupyter-repl/docs/Installation.html"),
            },
            {
              text: "Popular Kernels",
              onDidClick: () => openExternal("https://nteract.io/kernels"),
            },
            {
              text: "All Kernels",
              onDidClick: () =>
                openExternal("https://github.com/jupyter/jupyter/wiki/Jupyter-kernels"),
            },
          ],
        };
