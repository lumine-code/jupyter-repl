...
class ZMQKernel extends KernelTransport {
    ...
    try {
      ...
      if (error.message && error.message.includes("busy")) {
        ...
        if (callbackInfo) {
          ...
          // A caller awaiting the execution needs the trailing idle as well as
          // the reply before it resolves; the kernel never saw the request, so
          // synthesize that too.
          ...
          // The messages are synthesized here rather than received, so they
          // never reach the handlers that would retire the request. Release the
          // slot by hand: it admits one execution at a time, and leaving it
  ...
  onShellMessage(message) {
    ...
    if (msg_type === "execute_reply") {
      // Release the execution slot so the next queued request goes out.
      if (this.currentExecutionRequest === msg_id) {
        this.currentExecutionRequest = null;
        setImmediate(() => this._processExecutionQueue());
      }
