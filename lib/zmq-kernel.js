...
// callback entry is the authority while it exists, but it is gone by the time
// the trailing idle lands — the kernel replies before it publishes idle, and
// a reply retires the entry — and a foreign client's introspection has no
// entry here at all. Deliberately excludes `execute_request`, because a cell
    ...
    this._ackWatchdog = setInterval(() => {
      ...
      for (const [requestId, callbackInfo] of Object.entries(this.executionCallbacks)) {
        ...
        if (!replyDone && !idleDone) {
          ...
          // something and then went quiet mid-execution. Count only the
          // stretch in which the kernel was both idle and had the request:
          // idle since before it was sent, or ever since.
        ...
        // Exactly one answer arrived. Timed from the moment it did, never
        // from `_reportedIdleSince`: that is refreshed by every idle from
        // every client and by our own suppressed traffic, so a dragged slider
        // — one comm message per frame — would hold it at `now` and starve
        // this branch for as long as the drag lasted.
        if (now - callbackInfo.halfSettledAt <= ZMQKernel.HALF_SETTLED_GRACE_MS) {
          continue;
        }
        if (replyDone) {
