...
class ZMQKernel extends KernelTransport {
  ...
  // never on `store.startingKernels` — that map is keyed by display name, and
  // two kernels of one spec is the normal multi-file state.
  _everReady = false;
  ...
  monitorNotifications(childProcess) {
    ...
    childProcess.on("exit", (code, signal) => {
      ...
      if (!this._everReady) {
        ...
        // never by `store.startingKernels` — that map is keyed by display
        // name, and two kernels of one spec is the normal multi-file state.
        // Keyed on the name, a running kernel dying during its sibling's
        // startup window took this branch: no lose event, so its cells spun
        // forever, and the sibling's starting marker was deleted from under
        // it.
        store.startingKernels.delete(this.kernelSpec.display_name);
        if (code !== 0) {
          lumine.notifications.addError(`${this.kernelSpec.display_name}`, {
   ...
   * "restarting" forever, its display name stuck in `startingKernels` so
  ...
  _onReadyProbeExhausted() {
    ...
    // the name can only belong to a same-spec sibling still starting, and
    // deleting that reopens the duplicate-start guard under it.
    if (!this._everReady) {
      const store = require("./store");
      store.startingKernels.delete(this.kernelSpec.display_name);
    }
    ...
    // The exit this kill provokes is announced above; the handler must not
    // announce it a second time as a crash.
    this._expectingExit = true;
    try {
      this._kill();
    } catch (error) {
      log("ZMQKernel: Error killing the mute kernel:", error.message);
    }
  }
   ...
   */
  shutdown() {
    return (this._shutdownPromise ??= this._socketShutdown());
  }
  ...
  _socketRestart(onRestarted) {
    ...
    // A fresh process, a fresh account of its exits. Left over from an
    // exhaustion-killed predecessor, the flag would settle this one's death
    // as a quiet shutdown too.
    this._expectingExit = false;
  ...
  async _sendShellMessage(message, requestId, onResults, suppressStatus = false, options = {}) {
    ...
    // A released socket means a shutdown is in flight: settle cleanly rather
    // than letting the null ref surface as a TypeError in the caller's bubble.
    if (!this.shellSocket) {
  ...
  _startAckWatchdog() {
    ...
    this._ackWatchdog = setInterval(() => {
      ...
      for (const [requestId, callbackInfo] of Object.entries(this.executionCallbacks)) {
        ...
        // patient rule as everything else the kernel has not touched. The
        // stamp counts as arrival too: a comm's busy sets it, and that busy
        // is the kernel demonstrably attending.
