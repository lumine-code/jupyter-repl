...
class ZMQKernel extends KernelTransport {
  ...
  _startReadyProbe() {
    ...
    const probe = () => {
      ...
      this._queueShellMessage(
        _createMessage("kernel_info_request", requestId),
        requestId,
        () => {},
      );
    };
  ...
  onIOMessage(message) {
    ...
    if (callbackInfo) {
      ...
      // Route orphan output messages (from background threads) to registered output stores
      this._routeOrphanOutput(msg_id, message);
    }
   ...
   */
  _routeOrphanOutput(msgId, message) {
