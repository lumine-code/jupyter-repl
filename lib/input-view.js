...
const { surfaceForInputRoute, validateInputRoute } = require("./input-route");
 ...
 */
class InputView {
  constructor({ prompt, defaultText, allowCancel, password, route }, onConfirmed, onCancelled) {
    this.route = validateInputRoute(route);
    this.onConfirmed = onConfirmed;
    this.onCancelled = onCancelled;
    this.allowCancel = Boolean(allowCancel);
    this.disposables = new CompositeDisposable();
    this.surfaceDisposables = null;
    ...
    const surface = surfaceForInputRoute(this.route);
    ...
    if (this.allowCancel) {
      const handleBlur = () => {
        if (!this.panel.isTransferring() && this.element.ownerDocument.hasFocus()) this.cancel();
      };
    ...
    this.disposables.add(
      this.panel.onDidChangeVisible((visible) => {
        if (!visible && !this.panel.isTransferring() && !this.settled) this.cancel();
      }),
      this.panel.onDidDestroy(() => {
        if (!this.settled) this.cancel();
      }),
      this.panel.onDidChangeDocument(() => {
        if (!this.settled) this.bindSurface(surfaceForInputRoute(this.route));
      }),
    );
  ...
  bindSurface(surface) {
    if (this.boundSurface === surface) return;
    this.surfaceDisposables?.dispose();
    this.surfaceDisposables = new CompositeDisposable();
    this.boundSurface = surface;

    const cancel = () => this.cancel();
    surface.window.addEventListener("pagehide", cancel);
    surface.window.addEventListener("unload", cancel);
    this.surfaceDisposables.add(
      new Disposable(() => {
        surface.window.removeEventListener("pagehide", cancel);
        surface.window.removeEventListener("unload", cancel);
      }),
    );
  }
