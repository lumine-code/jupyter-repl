...
class KernelTransport {
  ...
  // Our connection's own lifecycle:
  ...
  constructor(kernelSpec, grammar) {
    ...
    // Local transports launch the process they wrap. Remote transports
    // override this when they merely attach to an already-running session so
    // automatic binding cleanup disconnects without deleting that session.
    this.ownsKernelProcess = true;
    this._languageInfo = null;
    this._emitter = new Emitter();
  }

  /**
   * The language reported by the running kernel wins over the kernelspec. A
   * kernelspec is discovery metadata; kernel_info_reply is the process telling
   * us what actually started.
   */
  get language() {
    return String(this._languageInfo?.name || this.kernelSpec?.language || "").toLowerCase();
  }

  /** The last language_info object reported by the running kernel. */
  get languageInfo() {
    return this._languageInfo;
  }

  setLanguageInfo(languageInfo) {
    if (!languageInfo || typeof languageInfo !== "object") return;
    this._languageInfo = Object.freeze({ ...languageInfo });
  }
