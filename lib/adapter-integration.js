...
const { ownerOrActiveSurfaceInputRoute } = require("./input-route");
const {
...
function runAdapterTargets(
  ...
  // A pane-backed adapter follows that item between surfaces. An adapter with
  // no pane item is background/plugin work, so it captures the active native
  // surface now rather than guessing again if stdin arrives later.
  const inputRoute = ownerOrActiveSurfaceInputRoute(adapter.getPaneItem?.());
    ...
    for (const target of targets) {
      ...
      try {
        executionPromise = result.createResultAsync(
          { editor, kernel: mappedKernel, markers, route: inputRoute },
          {
            code,
            row,
            cellType: "codecell",
