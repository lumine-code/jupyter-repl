...
//
// The drain goes through `lumine.views`, which is the scheduler
// `TextEditorComponent#scheduleUpdate` writes to: it batches every writer into
// one animation frame, so the flush runs before the frame's style, layout and
// paint and a bubble's growth still lands in the same painted frame. Two
...
//
// `lumine.views` by name, not this package's `etch.getScheduler()`: the
// package resolves its own copy of etch, whose scheduler is a private
// DefaultScheduler rather than the editor's view registry, so scheduling
// there would coordinate with nothing.
