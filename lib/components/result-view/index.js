...
class ResultView {
   ...
   * has changed. Gated three ways: the dirty flag stops the measurement
   * pass's own round trip from re-invalidating forever (parking the element
   * in the measuring area fires the resize observer, which re-runs the
   * render hooks); an attached bubble is skipped because the editor's own
   * resize observer already re-measures it in place; and a detached bubble
   * is throttled, with a trailing call so the last state always lands.
   */
