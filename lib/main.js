...
function provideAutocomplete() {
  autocompleteServiceFacade ||= createLazyService(() =>
...
function consumeStatusBar(statusBar) {
  return require("./services/consumed/status-bar/status-bar").statusBarConsumer.addStatusBar(
    store,
    statusBar,
  );
}
...
function consumeJupyterAdapter(service) {
  ...
  if (!adapterIntegrationActivated) {
    getAdapterIntegration().activateAdapterIntegration();
    adapterIntegrationActivated = true;
  }
  return new Disposable(() => {
