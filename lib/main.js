...
function activate() {
  emitter = new Emitter();
  require("./adapter-integration").activateAdapterIntegration();
...
function deactivate() {
  ...
  destroyOutputPanes();
  require("./adapter-integration").disposeAdapterIntegration();
  kernelPickerRequest++;
...
function connectToExistingKernel() {
  const adapterIntegration = require("./adapter-integration");
  const adapterContext = adapterIntegration.captureAdapterKernelContext(jupyterAdapterServices);
  if (adapterContext && !adapterIntegration.canChangeAdapterKernel(adapterContext)) return;
  ...
  if (!wsKernelPicker) {
    ...
    wsKernelPicker = new WSKernelPicker((transport, chosenContext) => {
      ...
      if (chosenContext?.adapter) {
        return adapterIntegration.bindAdapterKernel(chosenContext, kernel, { owned: true });
      }
