...
const { activePaneInputRoute, editorInputRoute } = require("./input-route");
...
function activate() {
  ...
  store.subscriptions.add(
    lumine.workspace.addWindowSurfaceTransitionObserver(
      require("./components/result-view/surface-transition").beginResultViewSurfaceTransition,
    ),
  );
...
function handleKernelCommand({ command, payload }, { kernel, markers }) {
  ...
  if (command === "interrupt-kernel") {
    ...
    if (kernel.transport instanceof require("./ws-kernel")) {
      kernel.transport.promptRename(activePaneInputRoute());
    } else {
...
function run(moveDown = false, event = null) {
  ...
  const inputRoute = editorInputRoute(editor);
  if (moveDown) {
  ...
  checkForKernel(store, async (kernel) => {
    ...
    const executionContext = { editor, kernel, markers: store.markers, route: inputRoute };
...
function runAllInline(event = null) {
  ...
  checkForKernel(store, async (kernel) => {
    ...
    await result.createResultBatch(
      { editor, kernel, markers: store.markers, route: inputRoute },
      codeBlocks,
    );
  });
}
