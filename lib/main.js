const { Emitter, CompositeDisposable, Disposable, Point } = require("atom");
...
function activate() {
  ...
  store.subscriptions.add(
    atom.config.onDidChange("jupyter-repl.languageMappings", ({ oldValue }) => {
      ...
      if (store.runningKernels.length !== 0) {
        ...
        atom.config.set("jupyter-repl.languageMappings", oldValue);
  ...
  store.subscriptions.add(
    ...
    // atom-text-editor:not([mini]) and .jupyter-notebook, which overlapped on
  ...
  store.subscriptions.add(
    atom.workspace.observeTextEditors((editor) => {
      ...
      if (atom.config.get("jupyter-repl.cellMarkers")) {
...
function terminateCommandEditorPendingState(event = null) {
  terminateEditorPendingState(
    event?.target?.closest?.("atom-text-editor")?.getModel?.() || store.editor,
  );
}
...
function debugToggle() {
  atom.config.set("jupyter-repl.debug", !atom.config.get("jupyter-repl.debug"));
}
...
function openExamples() {
  atom.app.openWindow({ pathsToOpen: __dirname + "/../examples" });
}
