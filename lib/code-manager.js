...
function getCodeToInspect(editor) {
  const { expression: code, cursorPosition } = getExpressionInfoAtCursor(editor);

  return [code, js_idx_to_char_idx(cursorPosition, code)];
}
...
// every call — O(file) apiece, which a bulk caller walking every row
// (run-all-inline) turned into an O(file²) freeze. The index records every
// match of the cell-marker regex in buffer order.
//
...
function getCell(editor, anyPointInCell) {
  ...
  log("CellManager: Cell [start, end]:", [start, end], "anyPointInCell:", anyPointInCell);
  return new Range(start, end);
}
...
function findCodeBlockAtRow(editor, row) {
  ...
  // 3. Check for fold-based blocks. The fold-range query is gated behind the
  // foldability check: for a non-foldable row the language mode answers
  // getFoldableRangeContainingPoint by walking backwards one row at a time to
  // the top of the file, so asking it about every plain statement line turns a
  // run-all-inline over a long file into an O(rows²) freeze — and the answer
  // was discarded anyway. A foldable row takes its fast path (the fold starts
  // on the row itself) and never walks.
...
function prepareCellDecoration(editor) {
  ...
  if (!buffer.hbpSubscription) {
    buffer.hbpMarkerLayer = buffer.addMarkerLayer({
      role: "jupyter-repl-breakpoints",
    });
  }
  editor.decorateMarkerLayer(editor.buffer.hbpMarkerLayer, {
    type: "line",
    class: "jupyter-repl-breakpoint",
  });
}
