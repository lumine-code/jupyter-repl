...
function provideMcpTools(getProvider) {
  ...
  return [
...

    {
      ...
      description:
        "Ask a running Jupyter kernel what it knows about an expression — the docstring, signature and type it would show on inspection. Returns {kernelId, status, found, text}, where status is 'ok', 'error' or 'timeout'. This does not run the code, so it is safe on an expression with side effects; use JupyterExecute when you need a value.",
      inputSchema: {
        type: "object",
        properties: {
          code: {
