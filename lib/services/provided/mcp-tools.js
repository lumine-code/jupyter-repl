...
// Introspection is not code being run, so it needs nothing like the budget an
// execution does — a kernel answers in milliseconds or it is not going to.
const DEFAULT_INSPECT_TIMEOUT_MS = 10000;
  ...
  return [
...

    {
      ...
      execute({ code, kernelId, timeoutMs = DEFAULT_TIMEOUT_MS, maxLength = DEFAULT_MAX_TEXT }) {
        ...
        // The schema says minimum 1, but nothing validates it, and 0 means
        // wait-forever one layer down — the very hang this deadline exists
        // to keep an assistant out of.
...

    {
      ...
      description:
        "Ask a running Jupyter kernel what it knows about an expression — the docstring, signature and type it would show on inspection. Returns {kernelId, status, found, text}, where status is 'ok', 'error' or 'timeout'; on 'error' an `error` string carries the reason. This does not run the code, so it is safe on an expression with side effects; use JupyterExecute when you need a value.",
      inputSchema: {
        type: "object",
        properties: {
          code: {
          ...
          timeoutMs: {
            type: "number",
            description: `How long to wait before giving up and reporting status 'timeout' (default ${DEFAULT_INSPECT_TIMEOUT_MS}).`,
            minimum: 1,
          },
          maxLength: {
            type: "number",
      ...
      execute({
        ...
        if (typeof code !== "string") throw new Error("code is required");
        if (!(timeoutMs > 0)) timeoutMs = DEFAULT_INSPECT_TIMEOUT_MS;
        return withKernel(kernelId, async (kernel) => {
          const { data, found, status, evalue } = await kernel.inspect(
            code,
          ...
          return {
            kernelId: kernel.id,
            // Without this, a kernel that restarted mid-request is reported
            // as "looked, knows nothing" — a claim about the code rather than
            // about the kernel, and the assistant has no way to tell.
            status: status || "ok",
            found: Boolean(found),
            ...(evalue ? { error: evalue } : {}),
...

    {
      ...
      execute({ kernelId } = {}) {
        return withKernel(kernelId, async (kernel) => {
          ...
          return { kernelId: kernel.id, restarted: restarted !== false };
        });
      },
    },
  ];
