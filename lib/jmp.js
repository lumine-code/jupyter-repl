...
const SOCKET_OPTIONS = {
  ...
  // A bounded native send is what lets close wait for it instead of ever
  // closing underneath it — an operation that corrupts libzmq on Windows.
  sendTimeout: 2000,
  receiveTimeout: -1,
};
...
function createZmqSocket(socketType, options = {}) {
  ...
  return configureSocket(socket, options);
}
...
function configureSocket(socket, options = {}) {
  ...
  try {
    ...
    if (socket.sendTimeout !== undefined) {
      socket.sendTimeout = opts.sendTimeout;
    }
    if (socket.receiveTimeout !== undefined) {
      socket.receiveTimeout = opts.receiveTimeout;
    }
  } catch (err) {
...
class Socket {
  constructor(socketType, scheme, key, options = {}) {
    const { monitor = true, ...socketOptions } = options;
    this._jmp = {
      scheme: scheme,
      key: key,
    ...
    this._socket = createZmqSocket(socketType, socketOptions);
    this._monitorEnabled = monitor !== false;
   ...
   * @param {Function} [isStale] - Consulted when the send's turn in the chain
   *   arrives. A queued message can outlive its purpose — the request it
   *   carries may have been settled while a full socket held the chain up —
   *   and delivering it then would make the kernel execute work its caller
   *   already saw fail. Return true to skip the send.
   */
    ...
    (async () => {
      try {
        ...
        if (!this._closed) {
          ...
          this._lastError = err;
          // Observer telemetry must degrade without vetoing an otherwise
          // healthy data socket. Receive-loop and protocol/handshake failures
          // continue to use the actionable `error` event.
          this._emitEvent("monitor:error", err);
        }
      }
    })();
  }
  ...
  _startReceiveLoop() {
    ...
    loop.then(
      () => {
        if (this._receiveLoop === loop) this._receiveLoop = null;
      },
      () => {
        if (this._receiveLoop === loop) this._receiveLoop = null;
      },
   ...
   *
   * Passing `discardPending` sets the native socket's linger to zero immediately
   * before close. It discards messages already accepted into ZeroMQ's outbound
   * queue without closing underneath an active JavaScript send.
   *
   * @param {Boolean} [forUnload=false]
   * @param {Boolean} [discardPending=false]
   * @returns {Promise<void>} Settles after the native socket has closed.
   */
  close(forUnload = false, discardPending = false) {
    if (this._closed) {
    ...
    const closeSocket = () => {
      if (discardPending) {
        try {
          ...
          // A closed or partially constructed socket may reject option writes.
          // Closing it is still the safest available teardown.
          log("SOCKET: LINGER RESET ERROR (ignored):", err);
        }
      }
      try {
        this._socket.close();
    ...
    // that wedges every socket created afterwards. sendTimeout bounds the
    // native operation; teardown waits for the promise rather than racing it.
    if (this._inFlightSend) {
      const settled = this._inFlightSend.catch(() => {});
      this._closePromise = settled.then(closeSocket);
    } else {
      closeSocket();
      this._closePromise = Promise.resolve();
    }
