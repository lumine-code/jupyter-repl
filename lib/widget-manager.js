...
const { documentFor, loadBundleFor } = require("./realm-runtime");
...
const widgetModulesByDocument = new WeakMap();

async function widgetModulesForDocument(value) {
  const domDocument = documentFor(value);
  if (domDocument === globalThis.document) return WIDGET_MODULES;
  let modules = widgetModulesByDocument.get(domDocument);
  if (!modules) {
    modules = loadBundleFor(domDocument, require.resolve("./vendor/jupyter-widgets-realm"), {
      global: "LumineJupyterWidgets",
    }).then((realmWidgets) => ({
      "@jupyter-widgets/base": realmWidgets.base,
      "@jupyter-widgets/controls": realmWidgets.controls,
      [OUTPUT_MODULE]: createOutputModule(realmWidgets.base),
      lumino: realmWidgets.lumino,
    }));
    widgetModulesByDocument.set(domDocument, modules);
  }
...
class LumineWidgetManager extends baseManager.ManagerBase {
  ...
  create_view(model, options = {}) {
    const domDocument = options.document;
    if (!domDocument || domDocument === globalThis.document) {
      return super.create_view(model, options);
    }
    const id = `surface-view-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const viewPromise = (model.state_change = model.state_change.then(async () => {
      ...
      const modules = await widgetModulesForDocument(domDocument);
      const module = modules[viewModule];
      if (!module) {
        throw new Error(
          `The widget module "${viewModule}" is not bundled for this window surface.`,
        );
      }
      const ViewType = module[viewName];
      if (!ViewType) throw new Error(`"${viewName}" is not exported by ${viewModule}.`);
      const view = new ViewType({
        model,
        options: this.setViewOptions({ ...options, document: domDocument }),
      });
      view.__lumineWidgetRuntime = modules.lumino;
      view.listenTo(model, "destroy", view.remove);
      await view.render();
      view.once("remove", () => {
        if (model.views) delete model.views[id];
      });
      return view;
    }));
    if (model.views) model.views[id] = viewPromise;
    return viewPromise;
  }
