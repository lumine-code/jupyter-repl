...
const { preserveScroll } = require("./scroll-keeper");
...
function getGlobalOutputStore(kernel, inline = true) {
  ...
  return lumine.config.get("jupyter-repl.outputAreaDefault") ||
    lumine.workspace.getPaneItems().find((item) => item instanceof OutputPane)
    ? kernel.outputStore
    : null;
}
 ...
 */
function createPendingResult({ editor, kernel, markers }, { row, cellType, showResult = null }) {
 ...
 */
async function createResultBatch(
  ...
  const pendingResults = codeBlocks.map((codeBlock) =>
    createPendingResult(executionContext, codeBlock),
  );
  ...
  try {
    for (let index = 0; index < codeBlocks.length; index++) {
      ...
      if (!success) {
        ...
        // error status so each remaining loading bubble becomes an X.
 ...
 */
function clearResults({ editor, kernel, markers }) {
  preserveScroll(editor, () => {
    if (markers) {
      markers.clear();
    }
...
function createResultAsync(
  { editor, kernel, markers },
  { code, row, cellType, onResult, showResult = null, inline = true, pendingResult = null },
) {
  return new Promise((resolve) => {
    ...
    const globalOutputStore = getGlobalOutputStore(kernel, inline);
        ...
        case "codecell": {
          const outputStore = inline
            ? pendingResult ||
              createPendingResult(
                { editor, kernel, markers },
                { row, cellType, showResult: shouldShowResult },
              )
            : null;
