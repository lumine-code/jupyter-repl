 ...
 */
async function createResultBatch({ editor, kernel, markers, route }, codeBlocks) {
  ...
  try {
    const executionContext = { editor, kernel, markers, route: inputRoute };
 ...
 */
function createResult({ editor, kernel, markers, route }, { code, row, cellType }) {
  if (!editor || !kernel || !markers) {
    return;
  }
  const inputRoute = validateInputRoute(route);
