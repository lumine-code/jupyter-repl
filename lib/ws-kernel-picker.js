...
class WSKernelPicker {
  ...
  async toggle(_kernelSpecFilter) {
    ...
    await this.gatewayList.setItems(gateways);
    await this.gatewayList.show();
  }
  ...
  async showAuthList() {
    await this.authList.setInfoMessage(`Authenticate with ${this._gatewayName}`);
    await this.authList.show({ crumb: "Authentication" });
  }
  ...
  async onAuthMethod(action) {
    ...
    await this.credentialDialog.setInfoMessage(`${label} for ${this._gatewayName}`);
  ...
  async onCredential(value) {
    if (!value) {
      await this.credentialDialog.setStatus({
        type: "error",
        message: `Enter a ${this._credentialKind}.`,
      });
      return;
    }
  ...
  async showSpecList(sessionInfo) {
    ...
    await this.specList.show({ crumb: "New session" });
  }
