...
class WSKernelPicker {
  constructor(onChosen) {
    ...
    const queryEditor = this.credentialDialog.refs.queryEditor;
  ...
  async onCredential(value) {
    if (!value) {
      await this.credentialDialog.update({
        errorMessage: `Enter a ${this._credentialKind}.`,
      });
      return;
    }
