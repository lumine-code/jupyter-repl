...
class MiddlewareAdapter {
  ...
  restart(onRestarted) {
    if (this._middleware.restart) {
      return this._middleware.restart(this._nextAsPluginType, onRestarted);
    } else {
      return this._next.restart(onRestarted);
    }
  }
...
class Kernel {
  ...
  get languageInfo() {
    return this.transport.languageInfo;
  }
   ...
   */
  shutdown() {
    this.transport.invalidatePendingRequests?.("Kernel shut down");
  ...
  restart(onRestarted) {
    ...
    this.transport.invalidatePendingRequests?.("Kernel restarted");
    // A middleware may have accepted an execution without forwarding it to
    // the transport. Precise transport settlements remove their facade
    // entries first; this catches only those facade-only leftovers.
    this.abortInFlight("Kernel restarted");
    const restarting = this.firstMiddlewareAdapter.restart(() => {
    ...
    return Promise.resolve(restarting).then((restarted) => restarted !== false);
  }
  ...
  inspect(code, cursorPos, onResults) {
    ...
    this.firstMiddlewareAdapter.inspect(code, cursorPos, (message, channel) => {
      ...
      // inspect caller would then receive a false claim. The documented shape
