...
const { validateInputRoute } = require("./input-route");
...
class Kernel {
  ...
  execute(code, onResults, route) {
    const inputRoute = validateInputRoute(route);
    const wrappedOnResults = this._wrapExecutionResultsCallback(onResults, inputRoute);
   ...
   * its own types.
   */
  _wrapExecutionResultsCallback(onResults, inputRoute) {
      ...
      if (channel === "shell") {
        ...
        try {
          inputView = new InputView(
            ...
            // A native window, pane item, or competing modal may disappear
          ...
          // The execution route was live when execute() began but may have
          // disappeared before the asynchronous input_request arrived.
