...
class JupyterProvider {
  ...
  getCellRange() {
    ...
    const cells = require("../main").getJupyterCellsService();
   ...
   *
   * The text-editor registry comes first, so this finds embedded editors in
   * any workspace window without probing a global Document. When focus has
   * moved away entirely (a command run from the menu), an adapter or the
   * active pane item still knows which embedded editor is current.
   *
   * @returns {TextEditor|null}
   */
  getFocusedEditor() {
    const focused = lumine.textEditors.getActiveTextEditor();
