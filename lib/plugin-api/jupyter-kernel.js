...
class JupyterKernel {
   ...
   * @return {Promise<Boolean>} Whether a fresh kernel became ready
   */
  restart(onRestarted) {
    this._assertNotDestroyed();
    return this._kernel.restart(onRestarted);
  }
  ...
  /*
   * The language_info object reported by kernel_info_reply, or null until the
   * running kernel has supplied one.
   */
  get languageInfo() {
    this._assertNotDestroyed();

    return this._kernel.languageInfo;
  }
