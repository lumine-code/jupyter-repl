...
class Store {
  ...
  newKernel(kernel, filePath, editor, grammar) {
    if (utils.isMultilanguageGrammar(editor.getGrammar())) {
  ...
  removeKernelKey(filePath) {
    if (!filePath || !this.kernelMapping.delete(filePath)) return;
    if (this._externalKernelContext?.filePath === filePath) {
  ...
  remapKernelKey(oldKey, newKey) {
    ...
    if (existing instanceof getKernel() && incoming instanceof getKernel()) {
      this.kernelMapping.set(newKey, incoming);
    } else if (existing && typeof existing.set === "function" && incoming) {
      if (incoming instanceof getKernel()) {
  ...
  deleteKernel(kernel) {
    ...
    const wasExternal = this._externalKernel === kernel;
    if (wasExternal) {
      this._externalKernel = null;
      this._externalKernelContext = null;
    }
    ...
    if (files.length > 0 || this.runningKernels.length !== previousCount || wasExternal) {
      this._emitKernelsChanged();
    }
  }
  ...
  getFilesForKernel(kernel) {
    ...
    return this.filePaths.filter((file) => {
      ...
      return kernelOrMap instanceof getKernel()
        ? kernelOrMap === kernel
